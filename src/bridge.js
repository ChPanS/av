...
export function handleHap(hap) {
  ...
  if (midi !== null) {
    ...
    // сустейн-синты (пэд/бас/лид) -> uPad; верхние голоса (пэд) весомее баса
    if (typeof v.s === 'string' && /saw|tri|squ|sine|gm_|super/.test(v.s)) {
      const w = midi >= 47 ? 1.0 : 0.5;
      uniforms.uPad = Math.min(1.2, uniforms.uPad + 0.25 * w);
    }
  }
